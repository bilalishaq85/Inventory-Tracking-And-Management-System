...
function ValidateForm(thisform) { with (thisform) {
  ...
  if(isBlank(cat_name, "'Category-Name'", 3)){return false}
  if(isBlank_relax2(cat_desc, "'Category Description'", 3)){return false}
  return true; }}
</script>
<!-- InstanceEndEditable -->
<style type="text/css">
...
{	$form_action="save";
	$myquery = "Insert into categories (cat_code, cat_name, cat_desc)";
	$myquery.= " VALUES (NULL , '".$_POST['cat_name']."', '".mysql_real_escape_string($_POST['cat_desc'])."')";  
...
{	$form_action="save2";
	$myquery = "Update categories set ";
	$myquery.= " cat_name = '" . $_POST['cat_name'] . "' ,"; 
	$myquery.= " cat_desc = '".mysql_real_escape_string($_POST['cat_desc'])."' ";
	$myquery.= " where cat_code in ( " . $_POST['hidden_code'] . " )";
	$result = mysql_query($myquery); //or die('Error: ' . mysql_error());
...
{	$form_action="delete"; 
	...
	$myquery = "Delete from categories where cat_code in (" . $delete_list . "0)";  
	$result = mysql_query($myquery); //or die('Error: ' . mysql_error());
...
{	$form_action="rowdelete";
	$myquery = "Delete from categories where cat_code in ( " . $_POST['hidden_code'] . " )";
	$result = mysql_query($myquery); // or die('Error: ' . mysql_error());
...
<form action="" method="post" name="detail" onsubmit="return ValidateForm(this)">
  <span class="head1">New Category Definition</span>
	<table border="0" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
  <tr>
    <td width="112"><div align="right">Category-Name:</div></td>
    <td width="319"><div align="left">
      <input name="cat_name" type="text" id="cat_name" accesskey="1" tabindex="1" size="35" maxlength="50" 
	  value="<?php if ($err != 0 ) {echo $_POST['cat_name'];} ?>" />
    </div></td>
  </tr>
  <tr>
    <td><div align="right">Description:</div></td>
    <td><div align="left">
      <input name="cat_desc" type="text" id="cat_desc" accesskey="2" tabindex="2" size="50" maxlength="100" 
	  value="<?php if ($err != 0 ) {echo htmlentities($_POST['cat_desc']);} ?>" />
    </div></td>
  </tr>
</table>
...
{	$form_action="rowedit";
	//$_POST['hidden_code'];
	$myquery = "Select cat_code, cat_name, cat_desc from categories where cat_code in ( " . $_POST['hidden_code'] . " )";
	$result = mysql_query($myquery); // or die('Error: ' . mysql_error());
  ...
  <span class="head1">Edit Category Definition</span>
	<table width="434" border="0" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
  <tr>
    <td width="112"><div align="right">Category-Name:</div></td>
    <td width="322"><div align="left">
      <input name="cat_name" type="text" id="cat_name" accesskey="1" tabindex="1" size="35" maxlength="50" 
	  		value="<?php if ($err == 0 ) {echo $row[1];} else { echo $_POST['cat_name'];} ?>" />
    </div></td>
  </tr>
  <tr>
    <td><div align="right">Description:</div></td>
    <td><div align="left">
      <input name="cat_desc" type="text" id="cat_desc" accesskey="2" tabindex="2" size="50" maxlength="100" 
  			value="<?php if ($err == 0 ) {echo htmlentities($row[2]);} else { echo htmlentities($_POST['cat_desc']);} ?>" />
    </div></td>
  </tr>
</table>
<table width="118" border="0">
...
$myquery = "SELECT * FROM categories order by cat_name"; 
//---------------Paging Header--------------------*
...
<span class="head1">Inventory Categories</span>
				  ...
				  <table width="690" class="listtable1">
                    <tr class="listheader1" height="22" >
                      ...
                      <td width="236"><div align="center">Category Name</div></td>
                      <td width="361"><div align="center">Description</div></td>
                    </tr>
<?php  
$i=-1;
