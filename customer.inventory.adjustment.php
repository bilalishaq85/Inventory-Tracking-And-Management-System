...
{	// Below lines gave error for array with just one element, i-e element[0];
	...
	var physical_qty = document.getElementsByName('physical_quantity[]');
	var adjustment_qty = document.getElementsByName('adjustment_quantity[]');
	var t_physical_qty=0;
	var t_adjustment_qty=0;
	if (physical_qty[i].value == '') { adjustment_qty[i].value = 0; return true; }
	if (parseInt(physical_qty[i].value,10) > 0) {t_physical_qty = parseInt(physical_qty[i].value,10)} else {t_physical_qty = 0;}
	t_adjustment_qty = t_physical_qty - parseInt(part_qty[i].value,10);
	if (t_adjustment_qty > 99999999) 
	{	alert('Adjustment quantity can not exceed 99,999,999. Please re-enter quantity ... '); 
		physical_qty[i].value=''; adjustment_qty[i].value=0;
	}
	else {adjustment_qty[i].value = t_physical_qty - parseInt(part_qty[i].value,10);}

}
function setFocus(i)
{	var physical_qty = document.list.elements["physical_quantity[]"];
	physical_qty[i].focus();
}
function openwindow() {
...
switch($login_option){
	...
	case 3:
		$cust_code=$user_code;
		$heading1 = "Physical vs Perpetual Inventory (Adjustments)";
		$heading3 = "Note: Physical Inventory figures will be <strong>ADJUSTED</strong> to your inventory on SAVE. ";
		$heading3.= "A log will be maintained for all adjustments. ";
		$heading3.= "Before proceding to the next page or changing parts classification, ";
		$heading3.= "either save or cancel it, otherwise your changes in PHYSICAL(INVENTORY) column, will be lost.";
		?>
...
{	$form_action="save";
	...
	{	//if (isset($_POST["rowselector_". ($i)])) {$t_rowselector=1; } else { $t_rowselector=0; }
		...
		if ($_POST['adjustment_quantity'][$i] > 0) {$t_action=3;} else {$t_action=4;}
		if (($_POST['adjustment_quantity'][$i] != 0) && ($_POST['cust_inv_code'][$i] < 1))
		{	$myquery = "Insert into customer_inventory (cust_inv_code, cust_code, part_code, part_quantity, timestamp)";
			...
			$myquery.= " " . $_POST['physical_quantity'][$i] . ", '$t_timestamp' )";
			...
			{	$myquery = "Insert into _cust_inv_log_$cust_code";
				...
				$myquery.= " " . abs($_POST['adjustment_quantity'][$i]) . ",";
				$myquery.= " $t_action ,";			
				$myquery.= " '$t_timestamp' )";			
				$result = mysql_query($myquery); // or die('Error: ' . mysql_error());
				//echo $myquery;
		...
		elseif (($_POST['adjustment_quantity'][$i] != 0) && ($_POST['cust_inv_code'][$i] > 1))
		{	$myquery = "Update customer_inventory set";
			$myquery.= " part_quantity = " . $_POST['physical_quantity'][$i] . ",";
...
<table width="851" class="listtable1" >
                    <tr class="listheader1" height="22" >
					  ...
					  <td width="75"><div align="center">Perpetual</div></td>
					  <td width="75"><div align="center">Physical</div></td>
					  <td width="75"><div align="center">Adjustment</div></td>
                    </tr>
<?php  
$i=-1;
...
?>
  ...
  <td class="listcell1" >
   ...
   <td class="listcell1">
     <div align="center">
       <input name="physical_quantity[]" type="text" id="" accesskey="2" size="9" maxlength="8" style="text-align: right"
	   tabindex="<?php echo $i ?>"
	   onkeypress="return onlyNumbers();"
	   onkeyup="calcTotal(<?php echo $i ?>);"
	   class="inputtxt1"
	   value="" />
      </div>
   </td>
   <td class="listcell1">
      <div align="center">
       <input name="adjustment_quantity[]" type="text" id="" accesskey="2"  size="10" maxlength="9" 
