...
function ValidateForm(thisform) { with (thisform) {
  ...
  if(isBlank(staff_name, "'Staff Name'", 3)){return false}
  if(isBlank(staff_id, "'Login ID'", 5)){return false}
  if(isBlank(staff_pwd, "'Password'", 5)){return false}
  return true; }}
</script>
<!-- InstanceEndEditable -->
<style type="text/css">
...
{	$form_action="save";
	...
	$myquery = "Select dist_code, dist_id from distributors where dist_id ='" . $_POST['staff_id'] . "'";
	$count = mysql_num_rows(mysql_query($myquery));
	if ($count>0){errHandler(1062); $_POST['add']=true; $err=1;}
	else
	{	$myquery = "Insert into distributors_staff (staff_code, dist_code, staff_name, staff_id, staff_pwd, active)";
		$myquery.= " VALUES (NULL , $dist_code, '".$_POST['staff_name']."', ";
		$myquery.= "'".$_POST['staff_id']."', '".$_POST['staff_pwd']."', $active2 )";
		$result = mysql_query($myquery); // or die('Error: ' . mysql_error());
...
{	$form_action="save2";
	...
	{	$myquery = "Update distributors_staff set ";
		$myquery.= " staff_name = '" . $_POST['staff_name'] . "' ,"; 
		$myquery.= " staff_id = '".$_POST['staff_id']."' ,";
		$myquery.= " staff_pwd = '".$_POST['staff_pwd']."' ,";
		$myquery.= " active = $active2";
		$myquery.= " where staff_code in ( " . $_POST['hidden_code'] . " )";
		$result = mysql_query($myquery); //or die('Error: ' . mysql_error());
...
{	$form_action="delete"; 
	...
	$myquery = "Delete from distributors_staff where staff_code in (" . $delete_list . "0)";  
	$result = mysql_query($myquery); //or die('Error: ' . mysql_error());
...
{	$form_action="rowdelete";
	$myquery = "Delete from distributors_staff where staff_code in ( " . $_POST['hidden_code'] . " )";
	//echo $myquery;
	$result = mysql_query($myquery); // or die('Error: ' . mysql_error());
...
<form action="" method="post" name="detail" onsubmit="return ValidateForm(this)">
  <span class="head1">New Staff Definition</span>
	<table width="326" >
  <tr>
    <td width="101"><div align="right">Staff-Name :</div></td>
    <td colspan="3"><div align="left">
      <input name="staff_name" type="text" id="staff_name" accesskey="1" tabindex="1" size="33" maxlength="30" 
	  value="<?php if ($err != 0 ) {echo $_POST['staff_name'];} ?>" />
    </div></td>
  </tr>
  ...
  <tr>
    ...
    <td width="120"><div align="left">
      <input name="staff_id" type="text" id="staff_id" accesskey="2" tabindex="2" size="20" maxlength="20" 
	  value="<?php if ($err != 0 ) {echo $_POST['staff_id'];} ?>" />
    </div></td>
    <td width="63"><div align="right">Active :</div></td>
    <td width="42"><div align="left">
      <input name="active" type="checkbox" id="active" accesskey="3" tabindex="3" 
  ...
  <tr>
    ...
    <td colspan="3"><div align="left">
      <input name="staff_pwd" type="password" id="staff_pwd" accesskey="4" tabindex="4" size="20" maxlength="10" 
	  value="<?php if ($err != 0 ) {echo $_POST['staff_pwd'];} ?>" />
    </div></td>
  </tr>
</table>
<table width="118" border="0">
  ...
  <tr>
    <td><button type="submit" name="save" accesskey="5"  ><img src="images/bb_save.png" alt="Save row" ></button></td>
    <td>&nbsp;</td>
	<td><button type="submit" name="cancel" accesskey="5" ><img src="images/bb_cancel.png" alt="Cancel new" ></button></td>
  </tr>
</table>
</form>
<hr width=90%>
...
{	$form_action="rowedit";
	//$_POST['hidden_code'];
	$myquery = "SELECT staff_code, dist_code, staff_name, staff_id, staff_pwd, active from distributors_staff";
	$myquery.= " where staff_code in ( " . $_POST['hidden_code'] . " )";
	$result = mysql_query($myquery); // or die('Error: ' . mysql_error());
  ...
  <span class="head1">Edit Staff Definition</span>
	<table width="311" border="0" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
  <tr>
    <td width="93"><div align="right">Staff-Name :</div></td>
    <td colspan="3"><div align="left">
      <input name="staff_name" type="text" id="staff_name" accesskey="1" tabindex="1" size="33" maxlength="30" 
	  value="<?php if ($err == 0 ) {echo $row[2];} else { echo $_POST['staff_name'];} ?>" />
    </div></td>
  </tr>
  ...
  <tr>
    ...
    <td width="125"><div align="left">
      <input name="staff_id" type="text" id="staff_id" accesskey="2" tabindex="2" size="20" maxlength="20" 
	  value="<?php if ($err == 0 ) {echo $row[3];} else { echo $_POST['staff_id'];} ?>" />
    </div></td>
    <td width="53"><div align="right">Active :</div></td>
    <td width="40"><div align="left">
      <input name="active" type="checkbox" id="active" accesskey="3" tabindex="3" 
	  <?php if ($err == 0 ) {if ($row[5]==1) {echo ' checked'; } }
  ...
  <tr>
    ...
    <td colspan="3"><div align="left">
      <input name="staff_pwd" type="password" id="staff_pwd" accesskey="4" tabindex="4" size="20" maxlength="10" 
	  value="<?php if ($err == 0 ) {echo $row[4];} else { echo $_POST['staff_pwd'];} ?>" />
    </div></td>
  </tr>
</table>
<table width="118" border="0">
  ...
  <tr>
    <td><button type="submit" name="save2" accesskey="5"  ><img src="images/bb_save.png" alt="Save changes" ></button></td>
    <td>&nbsp;</td>
	<td><button type="submit" name="cancel" accesskey="5" ><img src="images/bb_cancel.png" alt="Cancel edit" ></button></td>
  </tr>
</table>
</form>
<hr width=90%>
...
$myquery = "SELECT staff_code, dist_code, staff_name, staff_id, staff_pwd, active FROM distributors_staff";
$myquery.= " WHERE dist_code=$dist_code order by staff_name"; 
//---------------Paging Header--------------------*
...
<span class="head1">Staff List</span>
				  <input name="hidden_code" type="hidden" value="" />
				  <table width="580" class="listtable1" >
                    <tr class="listheader1" height="22" >
                      <td colspan="3"><div align="center">Action</div></td>
                      <td width="255"><div align="center">Staff Name</div></td>
                      <td width="195"><div align="center">Login-ID</div></td>
...
?>
  <tr height="20"  bgcolor=<?php echo $list_bgcolor ?>  >
   ...
   <td><div align="center"><?php echo iif($row[5]==1,'Yes', 'No'); ?></div>
