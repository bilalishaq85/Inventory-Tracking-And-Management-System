...
function ValidateForm(thisform) { with (thisform) {
  ...
  if(isBlank(type_name, "'Type-Name'", 3)){return false}
  if(isBlank_relax2(type_desc, "'Type Description'", 3)){return false}
  return true; 
}}
</script>
<!-- InstanceEndEditable -->
...
{	$_SESSION['action_allowed'] = "save";
	$form_action="save";
	$myquery = "Insert into types (type_code, type_name, type_desc)";
	$myquery.= " VALUES (NULL , '".$_POST['type_name']."', '".mysql_real_escape_string($_POST['type_desc'])."')";  
...
{	$form_action="save2";
	$myquery = "Update types set ";
	$myquery.= " type_name = '" . $_POST['type_name'] . "' ,"; 
	$myquery.= " type_desc = '".mysql_real_escape_string($_POST['type_desc'])."' ";
	$myquery.= " where type_code in ( " . $_POST['hidden_code'] . " )";
	$result = mysql_query($myquery); //or die('Error: ' . mysql_error());
...
{	$form_action="delete"; 
	...
	$myquery = "Delete from types where type_code in (" . $delete_list . "0)";  
	$result = mysql_query($myquery); //or die('Error: ' . mysql_error());
...
{	$form_action="rowdelete";
	$myquery = "Delete from types where type_code in ( " . $_POST['hidden_code'] . " )";
	$result = mysql_query($myquery); // or die('Error: ' . mysql_error());
...
{	$_SESSION['action_allowed'] = "save";
  ...
  <span class="head1">New Type Definition</span>
	<table width="411" border="0" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
  <tr>
    <td width="84">Type-Name:</td>
    <td width="327"><div align="left">
      <input name="type_name" type="text" id="type_name" accesskey="1" tabindex="1" size="35" maxlength="50" 
	  value="<?php if ($err != 0 ) {echo $_POST['type_name'];} ?>" />
    </div></td>
  </tr>
  <tr>
    <td>Description:</td>
    <td><div align="left">
      <input name="type_desc" type="text" id="type_desc" accesskey="2" tabindex="2" size="50" maxlength="100" 
	  value="<?php if ($err != 0 ) {echo htmlentities($_POST['type_desc']);} ?>" />
    </div></td>
  </tr>
</table>
...
{	$form_action="rowedit";
	//$_POST['hidden_code'];
	$myquery = "Select type_code, type_name, type_desc from types where type_code in ( " . $_POST['hidden_code'] . " )";
	$result = mysql_query($myquery); // or die('Error: ' . mysql_error());
  ...
  <span class="head1">Edit Type Definition</span>
	<table width="411" border="0" cellpadding="0" cellspacing="0" bordercolor="#FFFFFF">
  <tr>
    <td width="84">Type-Name:</td>
    <td width="327"><div align="left">
      <input name="type_name" type="text" id="type_name" accesskey="1" tabindex="1" size="35" maxlength="50" 
	  		value="<?php if ($err == 0 ) {echo $row[1];} else { echo $_POST['type_name'];} ?>" />
    </div></td>
  </tr>
  <tr>
    <td>Description:</td>
    <td><div align="left">
      <input name="type_desc" type="text" id="type_desc" accesskey="2" tabindex="2" size="50" maxlength="100" 
  			value="<?php if ($err == 0 ) {echo htmlentities($row[2]);} else { echo htmlentities($_POST['type_desc']);} ?>" />
    </div></td>
  </tr>
</table>
<table width="118" border="0">
...
$myquery = "SELECT * FROM types order by type_name"; 
//---------------Paging Header--------------------*
...
<span class="head1">Inventory Types</span>
				  ...
				  <table width="690" class="listtable1">
                    <tr class="listheader1" height="22" >
                      ...
                      <td width="254"><div align="center">Type Name</div></td>
                      <td width="352"><div align="center">Description</div></td>
                    </tr>
<?php  
$i=-1;
